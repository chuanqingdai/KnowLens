...
const PROMPT_MAX_CHARS = Number.parseInt(process.env.IMAGE2_PROMPT_MAX_CHARS || "2600", 10);
...
function clampPromptForImage(prompt: string) {
  ...
  const limit = Number.isFinite(PROMPT_MAX_CHARS) ? Math.max(1200, PROMPT_MAX_CHARS) : 2600;
...
function normalizeTasksFromPayload(payload: GenerateBatchPayload) {
  ...
  const normalized = tasks
    .map((task, idx) => {
      ...
      const aspectRatio = (task.aspectRatio || payload.ratio || "9:16").trim() || "9:16";
