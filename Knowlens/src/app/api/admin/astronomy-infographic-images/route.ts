...
import {
  ...
  type AstronomyInfographicTemplate,
} from "@/lib/astronomy-infographic-templates";
import {
...
const publicDir = path.join(process.cwd(), "public/images/infographic/astronomy");
...
async function readManifest(): Promise<Manifest> {
  ...
  const currentSlugs = new Set(getAstronomyInfographicTemplates().map((template) => template.slug));
  try {
...
async function writeManifest(manifest: Manifest) {
  const templates = getAstronomyInfographicTemplates();
...
function requestMeta(slug: string, provider: ProviderName) {
  ...
  const requestId = `astronomy-infographic-tuzi-50-${slug}-${nonce}`;
  return {
    requestId,
    extraBody: {
      requestId,
...
function buildRecord(template: AstronomyInfographicTemplate, image: {
...
export async function POST(req: NextRequest) {
  ...
  const allTemplates = getAstronomyInfographicTemplates();
  ...
  const templates = requestedSlugs.length
    ? requestedSlugs
        ...
        .filter((template): template is AstronomyInfographicTemplate => Boolean(template))
  ...
  for (const template of templates) {
    ...
    while (attempt < maxAttempts && !completed) {
      ...
      try {
        ...
        const imageFilename = `astronomy-${template.slug}.webp`;
        const storageKey = `infographic/astronomy/${imageFilename}`;
        await writeFile(path.join(publicDir, imageFilename), converted);
        const completedAt = new Date().toISOString();
        const record = buildRecord(template, {
          imageFilename,
          previewImagePath: `/images/infographic/astronomy/${imageFilename}`,
          previewImageUrl: `https://knowlens.ai/images/infographic/astronomy/${imageFilename}`,
          storageKey,
