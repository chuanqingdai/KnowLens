...
import {
  ...
  type HistoryInfographicTemplate,
} from "@/lib/history-infographic-templates";
import {
...
const publicDir = path.join(process.cwd(), "public/images/infographic/history");
const manifestPath = path.join(process.cwd(), "src/lib/history-infographic-generated-images.json");
type ProviderName = "tuzi" | "duomi";
...
async function readManifest(): Promise<Manifest> {
  ...
  const currentSlugs = new Set(getHistoryInfographicTemplates().map((template) => template.slug));
  try {
...
async function writeManifest(manifest: Manifest) {
  const templates = getHistoryInfographicTemplates();
...
function requestMeta(slug: string, provider: ProviderName) {
  const timestamp = new Date().toISOString();
  const nonce = `${Date.now()}-${Math.random().toString(36).slice(2, 10)}`;
  const requestId = `history-infographic-${provider}-100-${slug}-${nonce}`;
  return {
    requestId,
    extraBody: {
      requestId,
...
function buildRecord(template: HistoryInfographicTemplate, image: {
  ...
  return {
    ...template,
    batchId: `history-infographic-${image.provider}-100`,
    batchTopic,
    generationProvider: image.provider,
    generationStatus: "success",
    tuziRequestId: image.tuziRequestId,
    sourceType: `${image.provider}_generated`,
    cacheBypassed: true,
...
export async function POST(req: NextRequest) {
  ...
  const provider: ProviderName = body.provider === "tuzi" ? "tuzi" : "duomi";
  const config = buildImage2ProviderConfig(provider);
  if (!config) {
    return NextResponse.json({ ok: false, error: `Missing ${provider} image provider config in the running Next server environment.` }, { status: 500 });
  }

  await mkdir(publicDir, { recursive: true });
  let manifest = await readManifest();
  manifest.job.status = "generating_images";
  const allTemplates = getHistoryInfographicTemplates();
  ...
  const templates = requestedSlugs.length
    ? requestedSlugs
        ...
        .filter((template): template is HistoryInfographicTemplate => Boolean(template))
  ...
  for (const template of templates) {
    ...
    while (attempt < maxAttempts && !completed) {
      ...
      const meta = requestMeta(template.slug, provider);
      try {
        ...
        if (priorUrls.includes(result.imageUrl) && attempt < maxAttempts) continue;
        if (priorUrls.includes(result.imageUrl)) throw new Error("suspected provider cache hit: repeated temporary image URL");
        ...
        const imageFilename = `history-${template.slug}.webp`;
        const storageKey = `infographic/history/${imageFilename}`;
        await writeFile(path.join(publicDir, imageFilename), converted);
        const completedAt = new Date().toISOString();
        const record = buildRecord(template, {
          provider,
          imageFilename,
          previewImagePath: `/images/infographic/history/${imageFilename}`,
          previewImageUrl: `https://knowlens.ai/images/infographic/history/${imageFilename}`,
          storageKey,
        ...
        manifest = await readManifest();
        manifest.sourceUrls[template.slug] = [...(manifest.sourceUrls[template.slug] || []), result.imageUrl].slice(-5);
        ...
        manifest = await readManifest();
        manifest.errors = manifest.errors.filter((item) => item.slug !== template.slug);
