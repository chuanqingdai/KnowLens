...
const publicDir = path.join(process.cwd(), "public/images/infographic/industry-report");
...
type BatchGenerationJob = {
  id: string;
  topicInput: "Industry Professional Report Insights";
  categorySlug: "industry-report";
  categoryName: "Industry Report";
  totalRequested: 50;
  aspectRatioPlan: { "9:16": 50 };
  provider: "tuzi";
  batchSize: 5;
  forceFreshGeneration: true;
  disableImageCache: true;
  requireProfessionalReportSource: true;
  requireOriginalShortQuote: true;
  requireVerifiedNumbers: true;
  allowLongCopyrightedText: false;
...
async function readManifest(): Promise<Manifest> {
  ...
  const currentSlugs = new Set(getIndustryReportTemplates().map((template) => template.slug));
  try {
...
async function writeManifest(manifest: Manifest) {
  const templates = getIndustryReportTemplates();
...
export async function POST(req: NextRequest) {
  ...
  const templates = requestedSlugs.length
    ? requestedSlugs
        ...
        .filter((template): template is IndustryReportTemplate => Boolean(template))
  ...
  for (const template of templates) {
    ...
    while (attempt < maxAttempts && !completed) {
      ...
      try {
        ...
        const imageFilename = `industry-report-${template.slug}.webp`;
        const storageKey = `infographic/industry-report/${imageFilename}`;
        await writeFile(path.join(publicDir, imageFilename), converted);
        const completedAt = new Date().toISOString();
        const record = buildRecord(template, {
          imageFilename,
          previewImagePath: `/images/infographic/industry-report/${imageFilename}`,
          previewImageUrl: `https://knowlens.ai/images/infographic/industry-report/${imageFilename}`,
          storageKey,
