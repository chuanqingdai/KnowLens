...
import {
  ...
  type RecipeInfographicTemplate,
...
const publicDir = path.join(process.cwd(), "public/images/infographic/recipe");
...
async function writeManifest(manifest: Manifest) {
  const templates = getRecipeInfographicTemplates();
...
function buildRecord(template: RecipeInfographicTemplate, image: {
...
export async function POST(req: NextRequest) {
  ...
  const templates = getRecipeInfographicTemplates().slice(offset, offset + limit);
  ...
  for (const template of templates) {
    ...
    while (attempt < maxAttempts && !completed) {
      ...
      try {
        ...
        const imageFilename = `recipe-${template.slug}.webp`;
        const storageKey = `infographic/recipe/${imageFilename}`;
        await writeFile(path.join(publicDir, imageFilename), converted);
        const completedAt = new Date().toISOString();
        const record = buildRecord(template, {
          imageFilename,
          previewImagePath: `/images/infographic/recipe/${imageFilename}`,
          previewImageUrl: `https://knowlens.ai/images/infographic/recipe/${imageFilename}`,
          storageKey,
        ...
        const itemIndex = getRecipeInfographicTemplates().findIndex((item) => item.slug === template.slug) + 1;
  ...
  return NextResponse.json({
    ...
    total: getRecipeInfographicTemplates().length,
    job: manifest.job,
