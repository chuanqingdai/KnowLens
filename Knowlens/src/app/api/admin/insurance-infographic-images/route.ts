...
import {
  ...
  type InsuranceInfographicTemplate,
...
const publicDir = path.join(process.cwd(), "public/images/infographic/insurance");
...
type BatchGenerationJob = {
  id: string;
  topicInput: "Insurance Knowledge";
  categorySlug: "insurance";
  categoryName: "Insurance";
  totalRequested: 30;
  aspectRatioPlan: { "16:9": 5; "9:16": 25 };
  provider: "tuzi";
  batchSize: 5;
...
async function readManifest(): Promise<Manifest> {
  ...
  const currentSlugs = new Set(getInsuranceInfographicTemplates().map((template) => template.slug));
  try {
...
async function writeManifest(manifest: Manifest) {
  const templates = getInsuranceInfographicTemplates();
...
function buildRecord(
  template: InsuranceInfographicTemplate,
  image: {
...
export async function POST(req: NextRequest) {
  ...
  const templates = requestedSlugs.length
    ? requestedSlugs
        ...
        .filter((template): template is InsuranceInfographicTemplate => Boolean(template))
  ...
  for (const template of templates) {
    ...
    while (attempt < maxAttempts && !completed) {
      ...
      try {
        ...
        const imageFilename = `insurance-${template.slug}.webp`;
        const storageKey = `infographic/insurance/${imageFilename}`;
        await writeFile(path.join(publicDir, imageFilename), converted);
        const completedAt = new Date().toISOString();
        const record = buildRecord(template, {
          imageFilename,
          previewImagePath: `/images/infographic/insurance/${imageFilename}`,
          previewImageUrl: `https://knowlens.ai/images/infographic/insurance/${imageFilename}`,
          storageKey,
