...
import {
  ...
  type SexEducationInfographicTemplate,
} from "@/lib/sex-education-infographic-templates";
import {
...
const publicDir = path.join(process.cwd(), "public/images/infographic/sex-education");
...
type BatchGenerationJob = {
  id: string;
  topicInput: string;
  categorySlug: "sex-education";
  categoryName: "Sex Education";
  totalRequested: 20;
  aspectRatioPlan: { "16:9": 5; "9:16": 15 };
  provider: "tuzi";
  batchSize: 5;
...
async function readManifest(): Promise<Manifest> {
  ...
  const currentSlugs = new Set(getSexEducationInfographicTemplates().map((template) => template.slug));
  try {
...
async function writeManifest(manifest: Manifest) {
  const templates = getSexEducationInfographicTemplates();
...
function requestMeta(slug: string, provider: ProviderName) {
  ...
  const requestId = `sex-education-infographic-tuzi-20-${slug}-${nonce}`;
  return {
    requestId,
    extraBody: {
      requestId,
...
function buildRecord(template: SexEducationInfographicTemplate, image: {
...
export async function POST(req: NextRequest) {
  ...
  const allTemplates = getSexEducationInfographicTemplates();
  ...
  const templates = requestedSlugs.length
    ? requestedSlugs
        ...
        .filter((template): template is SexEducationInfographicTemplate => Boolean(template))
  ...
  for (const template of templates) {
    ...
    while (attempt < maxAttempts && !completed) {
      ...
      try {
        ...
        const imageFilename = `sex-education-${template.slug}.webp`;
        const storageKey = `infographic/sex-education/${imageFilename}`;
        await writeFile(path.join(publicDir, imageFilename), converted);
        const completedAt = new Date().toISOString();
        const record = buildRecord(template, {
          imageFilename,
          previewImagePath: `/images/infographic/sex-education/${imageFilename}`,
          previewImageUrl: `https://knowlens.ai/images/infographic/sex-education/${imageFilename}`,
          storageKey,
