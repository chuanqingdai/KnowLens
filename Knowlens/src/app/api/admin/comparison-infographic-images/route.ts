...
import {
  ...
  type ComparisonInfographicTemplate,
...
const publicDir = path.join(process.cwd(), "public/images/infographic/comparison");
...
type BatchGenerationJob = {
  id: string;
  topicInput: "Comparison Infographic";
...
async function readManifest(): Promise<Manifest> {
  ...
  const currentSlugs = new Set(getComparisonInfographicTemplates().map((template) => template.slug));
  try {
...
async function writeManifest(manifest: Manifest) {
  const templates = getComparisonInfographicTemplates();
...
function buildRecord(
  template: ComparisonInfographicTemplate,
  image: {
...
export async function POST(req: NextRequest) {
  ...
  const requestedSlugs = Array.isArray(body.slugs) ? body.slugs.filter(Boolean) : [];
  ...
  const templates = requestedSlugs.length
    ? getComparisonInfographicTemplates().filter((item) => requestedSlugs.includes(item.slug)).slice(0, limit)
    : getComparisonInfographicTemplates().slice(offset, offset + limit);
  ...
  await Promise.all(templates.map(async (template) => {
    ...
    while (attempt < maxAttempts && !completed) {
      ...
      try {
        ...
        const imageFilename = `comparison-${template.slug}.webp`;
        const storageKey = `infographic/comparison/${imageFilename}`;
        await writeFile(path.join(publicDir, imageFilename), converted);
        const completedAt = new Date().toISOString();
        const record = buildRecord(template, {
          imageFilename,
          previewImagePath: `/images/infographic/comparison/${imageFilename}`,
          previewImageUrl: `https://knowlens.ai/images/infographic/comparison/${imageFilename}`,
          storageKey,
        ...
        const itemIndex = getComparisonInfographicTemplates().findIndex((item) => item.slug === template.slug) + 1;
  ...
  return NextResponse.json({
    ...
    total: getComparisonInfographicTemplates().length,
    job: manifest.job,
