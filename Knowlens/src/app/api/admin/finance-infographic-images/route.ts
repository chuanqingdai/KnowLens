...
import {
  ...
  type FinanceInfographicTemplate,
...
const publicDir = path.join(process.cwd(), "public/images/infographic/financial-report");
...
type BatchGenerationJob = {
  id: string;
  topicInput: "Finance Report Insights";
  categorySlug: "financial-report";
  categoryName: "Financial Report";
  totalRequested: 50;
...
async function readManifest(): Promise<Manifest> {
  ...
  const currentSlugs = new Set(getFinanceInfographicTemplates().map((template) => template.slug));
  try {
...
async function writeManifest(manifest: Manifest) {
  const templates = getFinanceInfographicTemplates();
...
function buildRecord(
  template: FinanceInfographicTemplate,
  image: {
...
export async function POST(req: NextRequest) {
  ...
  const templates = requestedSlugs.length
    ? requestedSlugs
        ...
        .filter((template): template is FinanceInfographicTemplate => Boolean(template))
  ...
  for (const template of templates) {
    ...
    while (attempt < maxAttempts && !completed) {
      ...
      try {
        ...
        const imageFilename = `finance-${template.slug}.webp`;
        const storageKey = `infographic/financial-report/${imageFilename}`;
        await writeFile(path.join(publicDir, imageFilename), converted);
        const completedAt = new Date().toISOString();
        const record = buildRecord(template, {
          imageFilename,
          previewImagePath: `/images/infographic/financial-report/${imageFilename}`,
          previewImageUrl: `https://knowlens.ai/images/infographic/financial-report/${imageFilename}`,
          storageKey,
