...
import {
  ...
  type EarthScienceInfographicTemplate,
} from "@/lib/earth-science-infographic-templates";
import {
...
const publicDir = path.join(process.cwd(), "public/images/infographic/earth-science");
...
async function readManifest(): Promise<Manifest> {
  ...
  const currentSlugs = new Set(getEarthScienceInfographicTemplates().map((template) => template.slug));
  try {
...
async function writeManifest(manifest: Manifest) {
  const templates = getEarthScienceInfographicTemplates();
...
function requestMeta(slug: string, provider: ProviderName) {
  ...
  const requestId = `earth-science-infographic-tuzi-100-${slug}-${nonce}`;
  return {
    requestId,
    extraBody: {
      requestId,
...
function buildRecord(template: EarthScienceInfographicTemplate, image: {
...
export async function POST(req: NextRequest) {
  ...
  const allTemplates = getEarthScienceInfographicTemplates();
  ...
  const templates = requestedSlugs.length
    ? requestedSlugs
        ...
        .filter((template): template is EarthScienceInfographicTemplate => Boolean(template))
  ...
  for (const template of templates) {
    ...
    while (attempt < maxAttempts && !completed) {
      ...
      try {
        ...
        const imageFilename = `earth-science-${template.slug}.webp`;
        const storageKey = `infographic/earth-science/${imageFilename}`;
        await writeFile(path.join(publicDir, imageFilename), converted);
        const completedAt = new Date().toISOString();
        const record = buildRecord(template, {
          imageFilename,
          previewImagePath: `/images/infographic/earth-science/${imageFilename}`,
          previewImageUrl: `https://knowlens.ai/images/infographic/earth-science/${imageFilename}`,
          storageKey,
