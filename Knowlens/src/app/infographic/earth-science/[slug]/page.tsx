...
import {
  getEarthScienceInfographicTemplate,
...
export function generateStaticParams() {
  return getEarthScienceInfographicTemplates()
...
export async function generateMetadata({ params }: PageProps): Promise<Metadata> {
  ...
  const template = getEarthScienceInfographicTemplate(slug);
...
export default async function EarthScienceInfographicTemplatePage({ params }: PageProps) {
  const { slug } = await params;
  const template = getEarthScienceInfographicTemplate(slug);
  if (!template || template.generationStatus !== "success") notFound();

  const related = template.relatedTemplateIds
    .map((id) => getEarthScienceInfographicTemplates().find((item) => item.id === id && item.generationStatus === "success"))
    .filter(Boolean)
    .slice(0, 6);
    ...
    "@graph": [
      ...
      },
      {
        ...
        itemListElement: [
          ...
          { "@type": "ListItem", position: 3, name: "Earth Science", item: "https://knowlens.ai/earth-science-infographic-generator" },
      ...
      <main className="bg-[#f7f9fb] px-4 py-6 text-zinc-950 sm:px-6 lg:px-8">
        <div className="mx-auto max-w-6xl">
          <nav className="mb-4 flex flex-wrap gap-2 text-sm text-zinc-500">
            ...
            <span>/</span>
            <Link href="/earth-science-infographic-generator" className="hover:text-zinc-900">Earth Science</Link>
            <span>/</span>
          ...
          <section className="rounded-[28px] border border-zinc-200 bg-white p-4 shadow-[0_20px_60px_rgba(15,23,42,0.10)] sm:p-5">
            <div className="mx-auto flex max-w-4xl flex-col items-center text-center">
              <p className="inline-flex rounded-full border border-emerald-200 bg-emerald-50 px-3 py-1 text-xs font-semibold uppercase tracking-[0.18em] text-emerald-700">
                Earth Science Infographic Template
              </p>
              ...
              <div className="mt-6 flex w-full flex-col justify-center gap-3 sm:w-auto sm:flex-row">
                ...
                <Link href="/earth-science-infographic-generator" className="inline-flex h-11 items-center justify-center rounded-full border border-zinc-300 bg-white px-5 text-sm font-semibold text-zinc-800 hover:bg-zinc-50">
          ...
          <section className="mt-8 grid gap-x-10 gap-y-10 lg:grid-cols-2">
            <article className="border-t border-zinc-200 pt-5">
              <h2 className="text-2xl font-semibold tracking-tight">About this earth science infographic</h2>
          ...
          <section className="mt-6 rounded-2xl border border-zinc-200 bg-zinc-950 p-6 text-white shadow-sm">
            <h2 className="text-xl font-semibold tracking-tight">Create a similar earth science infographic</h2>
            <p className="mt-3 max-w-3xl text-sm leading-7 text-zinc-300">
              Use this prepared prompt to create a similar visual with KnowLens AI. The prompt keeps the topic, earth science structure, aspect ratio, and core learning points aligned with this example.
            </p>
          ...
          <section className="mt-8">
            <h2 className="text-2xl font-semibold tracking-tight">Related earth science infographic templates</h2>
