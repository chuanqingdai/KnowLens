...
export function generateStaticParams() {
  return getBiologyInfographicTemplates().map((template) => ({ slug: template.slug }));
}

export async function generateMetadata({ params }: PageProps): Promise<Metadata> {
  const { slug } = await params;
  const template = getBiologyInfographicTemplate(slug);
  if (!template) return {};
  return {
...
export default async function BiologyInfographicTemplatePage({ params }: PageProps) {
  const { slug } = await params;
  const template = getBiologyInfographicTemplate(slug);
  if (!template) {
    notFound();
  ...
  const related = template.relatedTemplateIds
    .map((id) => getBiologyInfographicTemplates().find((item) => item.id === id))
    .filter(Boolean)
    .slice(0, 6);
        ...
        <div className="mx-auto max-w-6xl">
          ...
          <section className="rounded-[28px] border border-zinc-200 bg-white p-4 shadow-[0_20px_60px_rgba(15,23,42,0.10)] sm:p-5">
            ...
            <figure className="mt-5 overflow-hidden rounded-3xl bg-white">
              <div className="flex w-full items-start justify-center rounded-2xl bg-white">
                <img
                  ...
                  alt={template.imageAlt}
                  width={template.sourcePreviewWidth}
                  height={template.sourcePreviewHeight}
                  draggable={false}
            ...
            <div className="mt-4 flex flex-wrap items-center gap-2 text-sm text-zinc-600">
              ...
              <span className="rounded-full border border-zinc-200 bg-zinc-50 px-3 py-1">{template.styleName.replace(" Infographic Style", "")}</span>
          ...
          <section className="mt-8 grid gap-x-10 gap-y-10 lg:grid-cols-2">
            <article className="border-t border-zinc-200 pt-5">
              <h2 className="text-2xl font-semibold tracking-tight">About this biology infographic</h2>
              <div className="mt-4 space-y-4 text-sm leading-7 text-zinc-700">
                {template.visibleDescription.split("\n").map((paragraph) => (
            ...
            <article className="border-t border-zinc-200 pt-5">
              ...
              <p className="mt-4 text-sm leading-7 text-zinc-700">{template.visualPrompt}</p>
            </article>
          ...
          <section className="mt-6 rounded-2xl border border-zinc-200 bg-zinc-950 p-6 text-white shadow-sm">
            <h2 className="text-xl font-semibold tracking-tight">Create a similar biology infographic</h2>
            <p className="mt-3 max-w-3xl text-sm leading-7 text-zinc-300">
              Use this prepared prompt to create a similar visual with KnowLens AI. The prompt keeps the topic, aspect ratio, visual direction, and core biology points aligned with this example.
            </p>
            <Link
              href={createSimilarHref}
              className="mt-5 inline-flex h-11 items-center rounded-full bg-white px-5 text-sm font-semibold text-zinc-950 hover:bg-zinc-100"
            >
              Create similar infographic
            </Link>
          </section>
          ...
          <section className="mt-8">
            <h2 className="text-2xl font-semibold tracking-tight">Related biology infographic templates</h2>
