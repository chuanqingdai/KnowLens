...
export function generateStaticParams() {
  return getRoadmapInfographicTemplates()
...
export async function generateMetadata({ params }: PageProps): Promise<Metadata> {
  ...
  const template = getRoadmapInfographicTemplate(slug);
...
export default async function RoadmapInfographicTemplatePage({ params }: PageProps) {
  const { slug } = await params;
  const template = getRoadmapInfographicTemplate(slug);
    ...
    "@graph": [
      ...
      },
      {
        ...
        itemListElement: [
          ...
          { "@type": "ListItem", position: 3, name: "Roadmap", item: "https://knowlens.ai/infographic/roadmap" },
      ...
      <main className="bg-[#f7f9fb] px-4 py-6 text-zinc-950 sm:px-6 lg:px-8">
        <div className="mx-auto max-w-6xl">
          <nav className="mb-4 flex flex-wrap gap-2 text-sm text-zinc-500">
            ...
            <span>/</span>
            <Link href="/infographic/roadmap" className="hover:text-zinc-900">Roadmap</Link>
            <span>/</span>
          ...
          <section className="rounded-[28px] border border-zinc-200 bg-white p-4 shadow-[0_20px_60px_rgba(15,23,42,0.10)] sm:p-5">
            <div className="mx-auto flex max-w-4xl flex-col items-center text-center">
              ...
              <div className="mt-6 flex w-full flex-col justify-center gap-3 sm:w-auto sm:flex-row">
                ...
                <Link href="/infographic/roadmap" className="inline-flex h-11 items-center justify-center rounded-full border border-zinc-300 bg-white px-5 text-sm font-semibold text-zinc-800 hover:bg-zinc-50">
                  Roadmap examples
                </Link>
              </div>
            </div>
          ...
          <section className="mt-8 grid gap-x-10 gap-y-10 lg:grid-cols-2">
            <article className="border-t border-zinc-200 pt-5">
              <h2 className="text-2xl font-semibold tracking-tight">About this roadmap infographic</h2>
          ...
          <section className="mt-6 rounded-2xl border border-zinc-200 bg-zinc-950 p-6 text-white shadow-sm">
            <h2 className="text-xl font-semibold tracking-tight">Create a similar roadmap infographic</h2>
            <p className="mt-3 max-w-3xl text-sm leading-7 text-zinc-300">
              Use this prepared prompt to create a similar roadmap visual with KnowLens AI. The prompt keeps the phase structure, milestone logic, mobile readability direction, and action hierarchy aligned with this example.
            </p>
          ...
          {related.length > 0 ? (
            <section className="mt-8">
              <h2 className="text-2xl font-semibold tracking-tight">Related roadmap infographic templates</h2>
