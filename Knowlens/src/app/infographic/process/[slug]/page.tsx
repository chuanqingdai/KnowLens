...
export function generateStaticParams() {
  return getProcessInfographicTemplates().map((template) => ({ slug: template.slug }));
}

export async function generateMetadata({ params }: PageProps): Promise<Metadata> {
  const { slug } = await params;
  const template = getProcessInfographicTemplate(slug);
  if (!template) return {};
  return {
...
export default async function ProcessInfographicTemplatePage({ params }: PageProps) {
  const { slug } = await params;
  const template = getProcessInfographicTemplate(slug);
  if (!template) notFound();

  const related = template.relatedTemplateIds
    .map((id) => getProcessInfographicTemplates().find((item) => item.id === id))
    .filter(Boolean)
    .slice(0, 6);
        ...
        <div className="mx-auto max-w-6xl">
          ...
          <section className="mt-8 grid gap-x-10 gap-y-10 lg:grid-cols-2">
            <article className="border-t border-zinc-200 pt-5">
              <h2 className="text-2xl font-semibold tracking-tight">About this process infographic</h2>
          ...
          <section className="mt-6 rounded-2xl border border-zinc-200 bg-zinc-950 p-6 text-white shadow-sm">
            <h2 className="text-xl font-semibold tracking-tight">Create a similar process infographic</h2>
            <p className="mt-3 max-w-3xl text-sm leading-7 text-zinc-300">
              Use this prepared prompt to create a similar visual with KnowLens AI. The prompt keeps the topic, process structure, aspect ratio, and core learning points aligned with this example.
            </p>
          ...
          <section className="mt-8">
            <h2 className="text-2xl font-semibold tracking-tight">Related process infographic templates</h2>
