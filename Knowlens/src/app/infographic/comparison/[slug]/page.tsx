...
import {
  getComparisonInfographicTemplate,
  getComparisonInfographicTemplates,
} from "@/lib/comparison-infographic-templates";
...
export function generateStaticParams() {
  return getComparisonInfographicTemplates()
...
export async function generateMetadata({ params }: PageProps): Promise<Metadata> {
  ...
  const template = getComparisonInfographicTemplate(slug);
...
export default async function ComparisonInfographicTemplatePage({ params }: PageProps) {
  const { slug } = await params;
  const template = getComparisonInfographicTemplate(slug);
  if (!template || template.generationStatus !== "success") notFound();

  const allTemplates = getComparisonInfographicTemplates();
    ...
    "@graph": [
      ...
      },
      {
        ...
        itemListElement: [
          ...
          { "@type": "ListItem", position: 3, name: "Comparison", item: "https://knowlens.ai/infographic/comparison" },
      ...
      <main className="bg-[#f7f9fb] px-4 py-6 text-zinc-950 sm:px-6 lg:px-8">
        <div className="mx-auto max-w-6xl">
          <nav className="mb-4 flex flex-wrap gap-2 text-sm text-zinc-500">
            ...
            <span>/</span>
            <Link href="/infographic/comparison" className="hover:text-zinc-900">Comparison</Link>
            <span>/</span>
          ...
          <section className="rounded-[28px] border border-zinc-200 bg-white p-4 shadow-[0_20px_60px_rgba(15,23,42,0.10)] sm:p-5">
            <div className="mx-auto flex max-w-4xl flex-col items-center text-center">
              ...
              <div className="mt-6 flex w-full flex-col justify-center gap-3 sm:w-auto sm:flex-row">
                ...
                <Link href="/infographic/comparison" className="inline-flex h-11 items-center justify-center rounded-full border border-zinc-300 bg-white px-5 text-sm font-semibold text-zinc-800 hover:bg-zinc-50">
                  Comparison examples
                </Link>
              </div>
            </div>
            ...
            <div className="mt-4 flex flex-wrap items-center gap-2 text-sm text-zinc-600">
              ...
              <span className="rounded-full border border-zinc-200 bg-zinc-50 px-3 py-1">Mobile-readable layout</span>
            </div>
          </section>

          <section className="mt-8 grid gap-x-10 gap-y-10 lg:grid-cols-2">
            <article className="border-t border-zinc-200 pt-5">
              <h2 className="text-2xl font-semibold tracking-tight">About this comparison infographic</h2>
              <p className="mt-4 text-sm leading-7 text-zinc-700">{template.aboutDescription}</p>
            </article>
            ...
            <article className="border-t border-zinc-200 pt-5">
              <h2 className="text-lg font-semibold tracking-tight">Use cases and audience</h2>
              <div className="mt-4 grid gap-4 sm:grid-cols-2">
                <div>
                  <h3 className="text-xs font-semibold uppercase tracking-[0.14em] text-zinc-500">Use cases</h3>
                ...
                <div>
                  <h3 className="text-xs font-semibold uppercase tracking-[0.14em] text-zinc-500">Target audience</h3>
                  <ul className="mt-3 space-y-2 text-sm text-zinc-700">
                    {template.targetAudience.map((item) => (
                      <li key={item} className="flex gap-2">
                        <span className="mt-2 h-1.5 w-1.5 shrink-0 rounded-full bg-zinc-400" />
                        <span>{item}</span>
                      </li>
          ...
          <section className="mt-6 rounded-2xl border border-zinc-200 bg-zinc-950 p-6 text-white shadow-sm">
            <h2 className="text-xl font-semibold tracking-tight">Create a similar comparison infographic</h2>
            <p className="mt-3 max-w-3xl text-sm leading-7 text-zinc-300">
              Use this prepared prompt to create a similar comparison visual with KnowLens AI. The prompt keeps the comparison structure, mobile readability direction, and decision logic aligned with this example.
            </p>
          ...
          {related.length > 0 ? (
            <section className="mt-8">
              <h2 className="text-2xl font-semibold tracking-tight">Related comparison infographic templates</h2>
