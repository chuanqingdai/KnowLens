...
function buildPptPrompt(
  ...
  if (!isChineseLanguage(outputLanguage)) {
    return [
      ...
      "Each body slide should have one main point, a short title, and concise body content.",
      ...
      "The image should not contain the full body text. Treat body as presenter context; visible slide text should be a large title plus at most 1-2 short labels.",
      "Each slide title must stay concise and concrete (roughly: Chinese 8-16 chars, English 3-8 words), not a sentence.",
...
function buildVideoPrompt(
  ...
  if (!isChineseLanguage(outputLanguage)) {
    return [
      ...
      "visual must be based on that frame's own narration/scene copy. Describe a dominant subject, scene, composition, action, or change; do not write only 'comparison chart' or 'flow diagram'.",
  ...
  return [
    ...
    "onScreenText should usually be empty or very short because generated images are not editable subtitle tracks.",
    "For body frames, prefer no on-screen text. Do not ask Prompt3 to draw subtitles, long labels, bullet lists, tiny annotations, or UI-like small text.",
    "visual 必须基于当前分镜自己的旁白/scene 文案生成。描述一个明确主体、场景、构图、动作或变化，不要只写“对比图”“流程图”这种抽象类型。",
