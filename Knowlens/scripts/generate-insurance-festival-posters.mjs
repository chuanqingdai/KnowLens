...
import { buildInsurancePosterPrompt } from "../src/lib/insurance-poster-prompt.js";
import { festivalTemplates } from "../src/lib/insurance-festival-templates.js";

const __filename = fileURLToPath(import.meta.url);
const __dirname = path.dirname(__filename);
const projectRoot = path.resolve(__dirname, "..");
const outputDir = path.join(projectRoot, "public", "insurance", "posters");
const manifestPath = path.join(outputDir, "duanwu-generation-manifest.json");
const providerTimeoutMs = Number.parseInt(process.env.INSURANCE_FESTIVAL_IMAGE_TIMEOUT_MS || "360000", 10);
...
async function main() {
  ...
  for (const template of festivalTemplates) {
    ...
    const prompt = buildInsurancePosterPrompt(template, template.primaryCategory);
    try {
