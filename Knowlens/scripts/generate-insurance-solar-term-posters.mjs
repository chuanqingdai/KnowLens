...
import { solarTermTemplates } from "../src/lib/insurance-solar-term-templates.js";

const __filename = fileURLToPath(import.meta.url);
const __dirname = path.dirname(__filename);
const projectRoot = path.resolve(__dirname, "..");
const outputDir = path.join(projectRoot, "public", "insurance", "posters");
const manifestPath = path.join(outputDir, "jieqi-generation-manifest.json");
const styleSourcePath = path.join(projectRoot, "src", "app", "insurance", "InsuranceTemplateGallery.tsx");
const providerTimeoutMs = Number.parseInt(process.env.INSURANCE_SOLAR_TERM_IMAGE_TIMEOUT_MS || "360000", 10);
...
const styleByImageSrc = {
  "jieqi-01": "watercolor-story",
  "jieqi-02": "warm-family",
  "jieqi-03": "medical-fresh",
  "jieqi-04": "flat-illustration",
  "jieqi-05": "handdrawn-care",
  "jieqi-06": "soft-3d",
  "jieqi-07": "data-explainer",
  "jieqi-08": "storybook-kids",
  "jieqi-09": "light-luxury",
  "jieqi-10": "glassmorphism",
};
...
async function main() {
  ...
  const results = [];
  const failures = [];
  for (const template of solarTermTemplates) {
