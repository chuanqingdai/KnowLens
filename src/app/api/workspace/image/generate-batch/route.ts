...
function buildPromptFromPayloadTask(task: NonNullable<GenerateBatchPayload["tasks"]>[number]) {
  ...
  return [
    ...
    "Mobile readability is mandatory: design for phone viewing first, keep titles and key labels large, reduce tiny text, and avoid dense annotation clusters.",
    "Prefer fewer, larger text blocks over many small labels so the main message stays glance-readable on a mobile screen.",
