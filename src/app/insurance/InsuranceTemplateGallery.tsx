...
export function InsuranceTemplateGallery({ templates, categories }: InsuranceTemplateGalleryProps) {
  ...
  const openMembership = () => {
    if (typeof window !== "undefined") {
      ...
      window.sessionStorage.setItem(MEMBERSHIP_SOURCE_KEY, "insurance_template_membership");
    ...
    <>
      ...
      <div className="columns-1 [column-gap:1rem] sm:columns-2 lg:columns-3 xl:columns-4">
        {filteredTemplates.map((template, index) => {
          ...
          return (
            <article
              ...
              className="group relative mb-4 block w-full cursor-pointer break-inside-avoid-column overflow-hidden rounded-xl border border-zinc-200 bg-white shadow-[0_10px_25px_rgba(15,23,42,0.05)] transition hover:border-zinc-300 hover:shadow-[0_14px_30px_rgba(15,23,42,0.08)]"
            >
              <div className="relative w-full overflow-hidden bg-zinc-100">
                <CasePreview template={template} />
                {premium ? (
                  <div className="absolute right-2.5 top-2.5 z-10 inline-flex h-7 w-7 items-center justify-center rounded-full bg-amber-400 text-zinc-950 shadow-[0_8px_20px_rgba(15,23,42,0.2)] ring-1 ring-white/70">
                ...
                <div className="absolute inset-x-0 bottom-3 flex justify-center px-3 opacity-0 transition group-hover:opacity-100">
                  <div className="flex flex-wrap items-center justify-center gap-2">
                    <button
                      ...
                      className="relative z-20 inline-flex h-10 items-center justify-center rounded-full bg-zinc-950 px-4 text-sm font-medium text-white shadow-[0_12px_26px_rgba(15,23,42,0.28)] transition hover:bg-zinc-800"
                    >
                      生成同款
                  ...
                  <button
                    ...
                    className="relative z-20 inline-flex h-10 items-center justify-center rounded-full bg-white px-4 text-sm font-medium text-zinc-950 shadow-[0_12px_26px_rgba(15,23,42,0.2)] transition hover:bg-zinc-100 disabled:cursor-not-allowed disabled:opacity-60"
                  >
              ...
              <div className="p-3">
                ...
                <h3 className="mt-3 text-[15px] font-medium leading-6 text-zinc-900">{template.title}</h3>
        ...
        {emptyCategoryCards.map((card) => (
          <article
            key={card.id}
            className="mb-4 block w-full break-inside-avoid-column overflow-hidden rounded-xl border border-dashed border-zinc-200 bg-white shadow-[0_10px_25px_rgba(15,23,42,0.04)]"
          >
        ...
        <div className="fixed inset-0 z-[9999] flex items-stretch justify-stretch p-0 sm:items-center sm:justify-center sm:p-5">
          ...
          <div className="relative grid h-dvh max-h-dvh w-full max-w-5xl grid-rows-[minmax(0,42dvh)_minmax(0,1fr)] overflow-hidden border border-zinc-200 bg-white shadow-[0_24px_70px_rgba(15,23,42,0.28)] sm:h-[92dvh] sm:max-h-[92dvh] sm:rounded-2xl lg:grid-cols-[minmax(420px,1fr)_500px] lg:grid-rows-none">
            <button
              ...
              aria-label="关闭"
              onClick={() => setActiveTemplate(null)}
              className="absolute right-3 top-3 z-10 inline-flex h-9 w-9 items-center justify-center rounded-full border border-zinc-200 bg-white text-zinc-500 shadow-sm transition hover:bg-zinc-100 hover:text-zinc-900"
            >
              <X size={16} />
            </button>
            ...
            <div className="order-2 min-h-0 overflow-y-auto p-4 pb-5 sm:p-5 lg:order-1">
              ...
              <h3 className="mt-1.5 pr-10 text-xl font-semibold tracking-tight text-zinc-950">生成同款</h3>
              ...
              <div className="mt-4 grid gap-3">
                ...
                <FieldBlock label="尺寸">
                  <div className="grid grid-cols-4 gap-1.5 rounded-lg border border-zinc-200 bg-zinc-50 p-1">
                    {aspectRatioOptions.map((ratio) => {
                      const active = templateForm.aspectRatio === ratio;
                      return (
                        <button
                          key={ratio}
                          type="button"
                          disabled={isGenerateActionBusy}
                          onClick={() => setTemplateForm((prev) => (prev ? { ...prev, aspectRatio: ratio } : prev))}
                          className={`h-8 rounded-md text-xs font-medium transition disabled:cursor-not-allowed disabled:opacity-60 ${
                            active
            ...
            <div className="order-1 flex min-h-0 flex-col overflow-hidden border-b border-zinc-200 bg-zinc-100 lg:order-2 lg:border-b-0 lg:border-l">
              ...
              <div className="border-t border-zinc-200 bg-white px-3 py-3">
                <button
                  ...
                  className="inline-flex h-10 w-full items-center justify-center rounded-full bg-zinc-950 px-4 text-sm font-medium text-white transition hover:bg-zinc-800 disabled:cursor-not-allowed disabled:opacity-50"
                >
      ...
      {membershipPaywallOpen && typeof document !== "undefined"
        ? createPortal(
            <PaywallDialog
              ...
              title="开通会员，解锁高级保险海报"
              description={`少量基础海报可免费使用。${membershipPaywallAction === "download" ? "下载高级海报" : "生成高级同款"}需要会员权限，开通后可使用更多高质量保险宣传图、产品说明、客户教育、续保提醒和理赔服务模板。`}
              confirmLabel="去开通会员"
              cancelLabel="稍后再说"
              compact
      ...
      {creditsPaywallOpen && typeof document !== "undefined"
        ? createPortal(
            <PaywallDialog
              ...
              title="积分不足，开通后继续生成"
              description={`当前生成同款海报需要 ${INSURANCE_POSTER_GENERATION_CREDITS} 积分${
                creditsPaywallBalance === null ? "" : `，你的当前余额为 ${creditsPaywallBalance} 积分`
              }。开通会员后，可持续使用 KnowLens.ai 生成海量保险宣传图、产品说明、客户教育和续保服务海报。`}
              confirmLabel="去开通会员"
              cancelLabel="稍后再说"
              compact
