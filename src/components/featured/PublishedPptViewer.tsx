...
export function PublishedPptViewer({
  ...
  const createSimilarHref = templatePrompt ? `/app?prompt=${encodeURIComponent(templatePrompt)}` : "/app";
  ...
  const copyCanonicalUrl = useCallback(async () => {
    if (!canonicalUrl || !navigator.clipboard?.writeText) {
  ...
  const handleShare = useCallback(async () => {
    try {
      if (canonicalUrl && "share" in navigator && typeof navigator.share === "function") {
  ...
  }, [canonicalUrl, copyCanonicalUrl, description, title]);
    ...
    <>
      <section className="overflow-hidden rounded-2xl border border-zinc-200 bg-white shadow-sm">
        <div className="flex flex-wrap items-center justify-between gap-3 border-b border-zinc-100 px-4 py-3">
          ...
          <div className="flex flex-wrap items-center gap-2">
            <button
              ...
              className="inline-flex h-9 items-center gap-1.5 rounded-xl border border-zinc-300 bg-white px-3 text-xs font-medium text-zinc-700 hover:bg-zinc-100"
            >
              <Link2 size={14} />
              {shareLabel}
            </button>
            <Link
              ...
              className="inline-flex h-9 items-center gap-1.5 rounded-xl bg-zinc-950 px-3 text-xs font-semibold text-white hover:bg-zinc-800"
            >
              <Sparkles size={14} />
        ...
        <div className="bg-zinc-950/95 p-2 sm:p-4">
          {selectedAsset ? (
            <figure
              className="relative mx-auto w-full max-w-[min(1200px,100%)] select-none"
            ...
            >
              <img
                ...
                alt={imageAlt || selectedAsset.title || title}
              ...
              <figcaption className="sr-only">
                {selectedAsset.title || title} - an infographic preview created with KnowLens AI.
              </figcaption>
            </figure>
          ) : (
