...
async function attachLatestVideoExportAssets(items: PublishedCaseRow[]) {
  const candidates = items.filter((item) => {
    ...
    const videoAsset = item.assets?.find(isPublishedVideoAsset);
    return !videoAsset || !videoAsset.width || !videoAsset.height;
  });
  ...
  for (const item of candidates) {
    ...
    const existingVideoAsset = item.assets?.find(isPublishedVideoAsset);
    if (existingVideoAsset) {
      existingVideoAsset.width ||= job.timeline?.width || null;
      existingVideoAsset.height ||= job.timeline?.height || null;
      continue;
    }
    ...
    if (videoSourceUrl) {
      ...
      if (videoBytes?.bytes?.length) {
        ...
        const assetValues = [
          ...
          copied.mimeType,
          copied.fileSize,
          latestVideoJob?.timeline?.width || null,
          latestVideoJob?.timeline?.height || null,
          null,
          0,
          pages.length,
          now,
