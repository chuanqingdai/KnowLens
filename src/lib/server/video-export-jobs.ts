...
function normalizeVideoFps(inputLabel: string, outputLabel: string, fps: number, extraFilters: string[] = []) {
  const filters = ["fps=" + fps, "settb=AVTB", "setpts=PTS-STARTPTS", "setsar=1", "format=yuv420p"];
  if (extraFilters.length) {
    filters.push(...extraFilters, "fps=" + fps, "settb=AVTB", "setpts=PTS-STARTPTS", "setsar=1", "format=yuv420p");
  }
  return `[${inputLabel}]${filters.join(",")}[${outputLabel}]`;
}

async function concatSegments(segmentPaths: string[], outputPath: string, inputFps = DEFAULT_FPS) {
  if (!segmentPaths.length) {
    throw new Error("No rendered video segments to concatenate.");
  }
  const fps = Math.max(1, Math.round(inputFps || DEFAULT_FPS));
  const inputArgs = segmentPaths.flatMap((segmentPath) => ["-i", segmentPath]);
  const normalizedInputs = segmentPaths
    .map((_, index) => {
      return (
        normalizeVideoFps(`${index}:v`, `v${index}`, fps) + ";" +
  ...
  const normalizedOutput = normalizeVideoFps("cv", "v", fps);
  await runFfmpeg(
    [
      ...inputArgs,
      "-filter_complex",
      `${normalizedInputs};${concatInputs}concat=n=${segmentPaths.length}:v=1:a=1[cv][a];${normalizedOutput}`,
      "-map",
      "[v]",
      "-map",
      "[a]",
...
async function concatSegmentsWithTransitions(input: {
  ...
  const videoInputs = segmentPaths.map((_, index) => {
    ...
    const padFilters =
      outgoingTransitionSec > 0
        ...
        : [];
    return normalizeVideoFps(`${index}:v`, `tv${index}`, fps, padFilters);
  });
  ...
  transitions.forEach((transition, index) => {
    ...
    const rawOutputLabel = `xraw${index}`;
    ...
    transitionFilters.push(
      `[${currentVideoLabel}][${nextVideoLabel}]xfade=transition=${transitionName}:duration=${durationSec.toFixed(
        3,
      )}:offset=${centeredOffsetSec.toFixed(3)}[${rawOutputLabel}];${normalizeVideoFps(rawOutputLabel, outputLabel, fps)}`,
    );
  ...
  const filterComplex = [
    ...
    ...audioInputs,
    ...transitionFilters,
    normalizeVideoFps(currentVideoLabel, "v", fps),
