...
async function synthesizeWithEdge(
  ...
  for (let attempt = 1; attempt <= EDGE_TTS_MAX_ATTEMPTS; attempt += 1) {
    try {
      return await synthesizeWithEdgeOnce(text, voice);
    } catch (error) {
      lastError = error;
      if (attempt < EDGE_TTS_MAX_ATTEMPTS) {
        await sleep(EDGE_TTS_RETRY_DELAY_MS);
      }
    }
  }
  throw lastError instanceof Error ? lastError : new Error("Edge TTS connection failed");
}

async function synthesizeWithEdgeOnce(
  text: string,
  voice: TtsVoiceConfig,
): Promise<TtsAudioResult> {
